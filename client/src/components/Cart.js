      ...
      {cart.cartItems.length === 0 ? (
        ...
        <div>
              ...
              cart.cartItems.map((cartItem) => (
                <div className="w-50 p-2 mt-4" key={cartItem.id}>
                  <div className="d-flex flex-column align-items-center">
                    <img src={cartItem.imgUrl} alt={cartItem.name} className="w-50 rounded" />
                      <div className='text-info mt-1'>{cartItem.title}</div>
                      <div className='text-white text-center'>{cartItem.description}</div>
                      ...
                      <div className="d-flex align-items-center">
                        <button className='btn btn-primary' onClick={() => handleAddToCart(cartItem)}>+</button>
                        <div className="text-info mx-4">{cartItem.cartQuantity}</div>
                        <button className='btn btn-primary' onClick={() => handleDecreaseCart(cartItem)}>-</button>
                      </div>
          ...
          <div className="d-flex justify-content-center my-4">
            <button className="btn btn-outline-secondary mx-4" onClick={() => handleClearCart()}>
            Vaciar carrito
            </button>
            <div className="d-flex align-items-center">
              <div className="d-flex align-items-center">
